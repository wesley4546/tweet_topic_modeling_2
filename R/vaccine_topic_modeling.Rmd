...
save(list = ls(), file = here::here("data","RDA Data", "vaccine_topic_modeling_data.rda"))
```
...
save_plot <- function(plot, h, w, extension) {
  ggsave(
    filename = here::here("output", "graphs", "vaccine", paste0(deparse(substitute(plot)), extension)),
    plot = plot,
    width = w,
    height = h
  )
...
save(list = ls(), file = "vaccine_topic_model.rda")
```
